"""Validate a backend runtime and execute its backend-aware TinyTalk tests."""
...
TEST_SUITES = {
    "neutts": ("tests", "--ignore=tests/integration", "-q"),
    "omnivoice": ("tests/test_omnivoice_backend.py", "tests/test_backend_api.py", "-q"),
    "tinytauk": ("tests/test_tinytauk_backend.py", "tests/test_backend_api.py", "-q"),
}


if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("Usage: check.py {neutts|omnivoice|tinytauk}")
    ...
    if output_file:
        with open(output_file, "a", encoding="utf-8") as output:
            output.write(f"checked-backend={backend}\n")
            output.write(f"test-suite={' '.join(suite)}\n")
